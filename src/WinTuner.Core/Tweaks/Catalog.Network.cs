...
public static partial class Catalog
{
    public static List<RegistryTweak> GetNetwork() => new List<RegistryTweak>
    {
        ...
        new()
        {
        Id = "network.disable-rss",
        Title = "Disable Receive Side Scaling",
        Description = "Turns off Receive Side Scaling (Tcpip Parameters EnableRSS=0), which normally distributes network receive processing across CPUs. Disabling can help on single-CPU or virtual machines but usually lowers throughput on modern multi-core systems. REQUIRES ELEVATION and a reboot.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        ...
        new()
        {
        Id = "network.disable-media-sense",
        Title = "Disable Media Sense (DHCP reconnect)",
        Description = "Stops the TCP/IP stack from detecting media connect/disconnect events (DisableMediaSense=1), which prevents Windows from dropping and renewing DHCP leases when a network adapter briefly flaps. The default (0) keeps media sensing on. REQUIRES ELEVATION and a reboot.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\Tcpip\Parameters",
        ValueName = "DisableMediaSense",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 1,
        DisabledValue = 0,
        DefaultValue = 0,
        ...
        new()
        {
        Id = "network.disable-task-offload",
        Title = "Disable TCP task offload",
        Description = "Turns off TCP/IP checksum and segmentation offload to the network adapter (Tcpip Parameters DisableTaskOffload=1), which can resolve corrupt-transfer and compatibility issues on faulty drivers at a small CPU cost. The default (0) offloads to the NIC. REQUIRES ELEVATION and a reboot.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\Tcpip\Parameters",
        ...
        new()
        {
        Id = "network.disable-ip-helper",
        Title = "Disable IP Helper (Teredo/6to4/ISATAP)",
        Description = "Sets the IP Helper service (iphlpsvc) Start value to 4 (disabled), turning off the IPv6 transition technologies Teredo, 6to4, and ISATAP. Reduces network attack surface and background tunneling. REQUIRES ELEVATION and a reboot. Reset deletes the value to restore the default service start.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\iphlpsvc",
        ValueName = "Start",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 4,
        DisabledValue = 3,
        ...
        new()
        {
        Id = "network.disable-netbt-node-type",
        Title = "Force peer-to-peer NetBT node type",
        Description = "Sets the NetBIOS-over-TCP node type to 2 (peer-to-peer, no broadcast/peer elections), reducing NetBIOS chatter on quiet networks (NetBT Parameters NodeType=2). The default (absent) is hybrid. Requires elevation; mostly cosmetic for name resolution.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\NetBT\Parameters",
        ValueName = "NodeType",
        ...
        Title = "Disable Teredo tunneling",
        Description = "Turns off the Teredo IPv6-over-IPv4 tunneling interface (iphlpsvc Teredo State=Disabled), closing an often-unused transition path. The default (absent/Auto) enables it as needed. Requires elevation; can affect some IPv6 connectivity scenarios.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\iphlpsvc\Teredo",
        ValueName = "State",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 0,
        DisabledValue = 1,
        ...
        new()
        {
        Id = "network.disable-6to4",
        Title = "Disable 6to4 tunneling",
        Description = "Disables the 6to4 IPv6 transition mechanism (iphlpsvc 6to4 State=Disabled), another unused tunneling path on most local networks. The default (absent) enables it. Requires elevation; reduces background network traffic.",
        Category = TweakCategory.Network,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Services\iphlpsvc\6to4",
        ValueName = "State",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 0,
        DisabledValue = 1,
