...
public static partial class Catalog
{
    public static List<RegistryTweak> GetPerformance() => new List<RegistryTweak>
    {
        ...
        new()
        {
        Id = "performance.disable-cpu-throttling-on-ac",
        Title = "Disable CPU throttling on AC power",
        Description = "Removes the idle CPU throttle cap while on AC power (power Policy1 893dee8e-2bef-41e0-89c6-b55d0929964c=100%) so the processor can use its full range. The default (5%) parks cores under light load. Requires elevation; can increase power draw and heat.",
        Category = TweakCategory.Performance,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47ec7da1bd80\893dee8e-2bef-41e0-89c6-b55d0929964c",
        ...
        new()
        {
        Id = "performance.disable-dynamic-tick",
        Title = "Disable dynamic tick",
        Description = "Forces the system timer to run at a fixed interval instead of coalescing ticks (Boot Configuration DynamicTick=0), which can improve timer-sensitive workloads like audio/real-time apps at a small idle-power cost. The default (1) uses dynamic ticks. Requires elevation and a reboot.",
        Category = TweakCategory.Performance,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Control\Session Manager\kernel",
        ValueName = "DynamicTick",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 0,
        DisabledValue = 1,
        DefaultValue = 1,
        ...
        new()
        {
        Id = "performance.enable-ftrace-boot",
        Title = "Enable boot tracing (ETW logger)",
        Description = "Turns on the built-in boot ETW logger (GlobalLogger GlobalLogger=1) so Windows records startup events for diagnosis. The default (0) keeps it off. Requires elevation; produces a periodic kernel trace file. Mostly useful for performance investigation.",
        Category = TweakCategory.Performance,
        Hive = RegistryHive.LocalMachine,
        SubKey = @"SYSTEM\CurrentControlSet\Control\WMI\GlobalLogger",
        ValueName = "Start",
        ValueKind = RegistryValueKind.DWord,
        EnabledValue = 1,
        DisabledValue = 0,
